...
### 2025-10-14 — Phase 2: NEON backend modernization — COMPLETE

- Context
  - Align NEON backend with SSE2/AVX2 modernization: std::span APIs, C++20 popcount, and stylistic cleanup.
- Changes
  - Added span-based BindWords/HammingWords as primary APIs; retained pointer forwarders.
  - Removed unnecessary reinterpret_cast in vld1q_u64/vst1q_u64; used direct pointers from spans.
  - Introduced Popcount64 using std::popcount with builtin fallback.
  - Identifier renames and fixed-width types to satisfy zero-lint (no NOLINTs).
- Results
  - clang-tidy: clean relative to enforced checks; no reinterpret_cast/identifier-length/runtime-int issues.
  - Build: PASS; Tests: PASS (87/87). macOS arm64 CI will exercise NEON path.
- Files
