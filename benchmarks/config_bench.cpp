...
void ReportFootprints() {
  ...
  std::printf(
      "Footprints/"
      "BinaryHV(dim=%zu)=%zub,ClusterMemory(dim=%zu,cap=16)=%zub,PrototypeMemory(dim=%zu,cap=256)=%"
      "zub\n",
...
static std::pair<double, double> MicrobenchHammingSSE2vsAVX2(std::size_t iters) {
  ...
  for (std::size_t i = 0; i < iters; ++i)
    sink += hyperstream::backend::sse2::HammingDistanceSSE2<Dim>(a, b);
  const auto t1 = high_resolution_clock::now();
  // AVX2 (if compiled)
  const auto t2 = high_resolution_clock::now();
#if defined(__AVX2__)
  for (std::size_t i = 0; i < iters; ++i)
    sink += hyperstream::backend::avx2::HammingDistanceAVX2<Dim>(a, b);
#endif
...
int main(int argc, char** argv) try {
  ...
  std::printf(
      "Config/profile=%s,default_dim_bits=%zu,default_capacity=%zu,heap_threshold_bytes=%zu\n",
      hyperstream::config::kActiveProfile, hyperstream::config::kDefaultDimBits,
      hyperstream::config::kDefaultCapacity, hyperstream::config::kHeapAllocThresholdBytes);

  // CPU features
  const std::uint32_t mask = hyperstream::backend::GetCpuFeatureMask();
  std::printf(
      "CPUFeatures/mask=0x%08x,SSE2=%d,AVX2=%d\n", mask,
  ...
  std::printf("Policy/HammingThreshold=%zu,overridden=%d\n", thr,
              hyperstream::backend::HammingThresholdOverridden() ? 1 : 0);
  ...
  if (auto_tune) {
    ...
    struct Case {
      std::size_t dim;
      std::size_t iters;
    } cases[] = {{8192, 8000}, {16384, 4000}, {32768, 2000}, {65536, 1000}};
    ...
    for (const auto& c : cases) {
      ...
      switch (c.dim) {
        case 8192: {
          auto p = MicrobenchHammingSSE2vsAVX2<8192>(c.iters);
          sse2_ms = p.first;
          avx2_ms = p.second;
          break;
        }
        case 16384: {
          auto p = MicrobenchHammingSSE2vsAVX2<16384>(c.iters);
          sse2_ms = p.first;
          avx2_ms = p.second;
          break;
        }
        case 32768: {
          auto p = MicrobenchHammingSSE2vsAVX2<32768>(c.iters);
          sse2_ms = p.first;
          avx2_ms = p.second;
          break;
        }
        case 65536: {
          auto p = MicrobenchHammingSSE2vsAVX2<65536>(c.iters);
          sse2_ms = p.first;
          avx2_ms = p.second;
          break;
        }
