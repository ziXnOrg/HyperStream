...
TEST_F(CoreOpsTest, BindingProperties) {
  ...
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_EQ(result_.GetBit(i), result2.GetBit(i)) << "Binding operation should be commutative";
  }
  ...
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_EQ(result_.GetBit(i), result2.GetBit(i)) << "Binding operation should be associative";
  }
}
...
TEST_F(CoreOpsTest, BundlingProperties) {
  ...
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_EQ(result_.GetBit(i), result2.GetBit(i)) << "Bundling operation should be commutative";
  }
}
...
TEST_F(CoreOpsTest, PermutationProperties) {
  ...
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_EQ(hv1_.GetBit(i), result_.GetBit(i)) << "Permutation by 0 should be identity";
  }

  // Test that permutation by dimension is identity (full rotation)
  PermuteRotate(hv1_, kTestDimension, &result_);
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_EQ(hv1_.GetBit(i), result_.GetBit(i)) << "Permutation by dimension should be identity";
  }
  ...
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_EQ(expected.GetBit(i), result_.GetBit(i)) << "Permutations should compose correctly";
  }
}
...
TEST_F(CoreOpsTest, SimilarityMeasures) {
  ...
  const float sim_ab = NormalizedHammingSimilarity<kTestDimension>({&hv1_, &hv2_});
  ...
  const float sim_aa = NormalizedHammingSimilarity<kTestDimension>({&hv1_, &hv1_});
...
TEST_F(CoreOpsTest, EdgeCases) {
  ...
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_EQ(hv1_.GetBit(i), result_.GetBit(i)) << "Binding with zero vector should be identity";
  }
  ...
  for (size_t i = 0; i < kTestDimension; ++i) {
    EXPECT_NE(hv1_.GetBit(i), result_.GetBit(i)) << "Binding with ones should flip all bits";
  }
...
TEST_F(CoreOpsTest, BindIsInvertibleWithKey_FixedSeed) {
  // bind(bind(a, key), key) == a
  HyperVector<kTestDimension, bool> bound;
  Bind(hv1_, hv2_, &bound);
  HyperVector<kTestDimension, bool> recovered;
  Bind(bound, hv2_, &recovered);
  for (std::size_t w = 0; w < recovered.Words().size(); ++w) {
    EXPECT_EQ(recovered.Words()[w], hv1_.Words()[w]) << "word index " << w;
  }
}

TEST_F(CoreOpsTest, HammingTriangleInequality_FixedSeed) {
  const auto d12 = HammingDistance(hv1_, hv2_);
  const auto d23 = HammingDistance(hv2_, hv3_);
  const auto d13 = HammingDistance(hv1_, hv3_);
  EXPECT_LE(d13, d12 + d23);
}
