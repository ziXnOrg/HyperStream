...
TEST(GoldenSerialization, Prototype_MatchesCommittedFixture_D128_C4) {
  ...
  HyperVector<128, bool> q; q.Clear();
...
static std::string ToHex(const std::array<std::uint8_t,32>& d){
  ...
  for(int i=0;i<32;++i){ s[i*2]=k[(d[i]>>4)&0xF]; s[i*2+1]=k[d[i]&0xF]; }
  return s;
}

static std::string Sha256Hex(const std::vector<std::uint8_t>& v){ return ToHex(Sha256(v)); }
...
TEST(GoldenSerialization, Manifest_Sha256MatchesFiles) {
  ...
  const char* files[] = {
    "prototype_d96_c3.hser1",
    "prototype_d128_c4.hser1",
    "cluster_d96_c3.hser1",
    "cluster_d128_c4.hser1"
  };
...
TEST(GoldenSerialization, DISABLED_GenerateHser1Fixtures) {
  ...
  {
    PrototypeMemory<96,3> mem;
    HyperVector<96,bool> a; a.Clear(); for (std::size_t i=0;i<96;i+=3) a.SetBit(i,true); ASSERT_TRUE(mem.Learn(1001, a));
    HyperVector<96,bool> b; b.Clear(); for (std::size_t i=1;i<96;i+=4) b.SetBit(i,true); ASSERT_TRUE(mem.Learn(2002, b));
    std::stringstream ss(std::ios::in|std::ios::out|std::ios::binary); ASSERT_TRUE(SavePrototype(ss, mem));
    const auto bytes = StreamBytes(ss);
    std::ofstream out(FixturePath("prototype_d96_c3.hser1"), std::ios::binary); out.write(reinterpret_cast<const char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
  }
  // Prototype 128/4 with 3 entries
  {
    PrototypeMemory<128,4> mem;
    HyperVector<128,bool> a; a.Clear(); for (std::size_t i=0;i<128;i+=3) a.SetBit(i,true); ASSERT_TRUE(mem.Learn(1001, a));
    HyperVector<128,bool> b; b.Clear(); for (std::size_t i=1;i<128;i+=4) b.SetBit(i,true); ASSERT_TRUE(mem.Learn(2002, b));
    HyperVector<128,bool> c; c.Clear(); for (std::size_t i=2;i<128;i+=5) c.SetBit(i,true); ASSERT_TRUE(mem.Learn(3003, c));
    std::stringstream ss(std::ios::in|std::ios::out|std::ios::binary); ASSERT_TRUE(SavePrototype(ss, mem));
    const auto bytes = StreamBytes(ss);
    std::ofstream out(FixturePath("prototype_d128_c4.hser1"), std::ios::binary); out.write(reinterpret_cast<const char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
  }
  // Cluster 96/3 with deterministic updates
  {
    ClusterMemory<96,3> mem;
    for (int rep=0; rep<5; ++rep){ HyperVector<96,bool> a; a.Clear(); for (std::size_t i=0;i<96;i+=3) a.SetBit(i,true); ASSERT_TRUE(mem.Update(1001, a)); }
    for (int rep=0; rep<3; ++rep){ HyperVector<96,bool> b; b.Clear(); for (std::size_t i=1;i<96;i+=4) b.SetBit(i,true); ASSERT_TRUE(mem.Update(2002, b)); }
    std::stringstream ss(std::ios::in|std::ios::out|std::ios::binary); ASSERT_TRUE(SaveCluster(ss, mem));
    const auto bytes = StreamBytes(ss);
    std::ofstream out(FixturePath("cluster_d96_c3.hser1"), std::ios::binary); out.write(reinterpret_cast<const char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
  }
  // Cluster 128/4 with deterministic updates
  {
    ClusterMemory<128,4> mem;
    for (int rep=0; rep<4; ++rep){ HyperVector<128,bool> a; a.Clear(); for (std::size_t i=0;i<128;i+=3) a.SetBit(i,true); ASSERT_TRUE(mem.Update(1001, a)); }
    for (int rep=0; rep<6; ++rep){ HyperVector<128,bool> b; b.Clear(); for (std::size_t i=1;i<128;i+=4) b.SetBit(i,true); ASSERT_TRUE(mem.Update(2002, b)); }
    std::stringstream ss(std::ios::in|std::ios::out|std::ios::binary); ASSERT_TRUE(SaveCluster(ss, mem));
    const auto bytes = StreamBytes(ss);
    std::ofstream out(FixturePath("cluster_d128_c4.hser1"), std::ios::binary); out.write(reinterpret_cast<const char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
  }
}
