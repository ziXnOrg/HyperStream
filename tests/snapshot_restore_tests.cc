...
TEST(SnapshotRestore, DISABLED_DumpSnapshots) {
  ...
  for (std::size_t i = 0; i < ord.size(); ++i) {
    ...
    if (idx == 16 || idx == 32 || idx == 48) {
      ...
      HyperVector<Pipeline::D, bool> tmp;
      p.cmem.Finalize(1, &tmp);
      std::printf("checkpoint@%zu immediate=%s\n", idx,
                  Hex64(HashWords(tmp.Words().data(), tmp.Words().size()) ^ p.mix).c_str());
      ...
      // Save sidecar state (last_obs words + mix) as tiny JSON
      {
        ...
        for (std::size_t w = 0; w < words.size(); ++w) {
          ...
          std::snprintf(buf, sizeof(buf), "\"0x%016llx\"",
                        static_cast<unsigned long long>(words[w]));
          os << buf;
...
TEST(SnapshotRestore, Parity_MultipleSnapshotPoints) {
  ...
  std::stable_sort(ord.begin(), ord.end(), TotalOrderLt);
  const int K = 16;
  const std::vector<int> points{16, 32, 48};
  ...
  std::vector<std::string> exp_chk_hex;
  std::string exp_final_hex;
  ...
  for (int N : points) {
    // Diagnostic: verify snapshot_N immediate hash equals uninterrupted CheckpointHash@N
    {
      ...
      }
      {
        ...
        while (ppos < b && widx < p0.last_obs.Words().size()) {
          ...
          auto qn = s.find('"', q + 1);
          if (qn == std::string::npos || qn > b) break;
    ...
    // Load sidecar state
    {
      ...
      ASSERT_NE(mpos, std::string::npos);
      std::size_t start = mpos + 7;
      if (start < s.size() && s[start] == '"') ++start;  // move past opening quote if present
      auto end = s.find('"', start);
      ASSERT_NE(end, std::string::npos);
      const std::string mix_hex = s.substr(start, end - start);
    ...
    if (N == 16) {
      ...
      // load sidecar again
      {
        ...
        auto m2 = s2.find("\"mix\":\"");
        std::size_t st = m2 + 7;
        if (st < s2.size() && s2[st] == '"') ++st;
        auto en = s2.find('"', st);
        const std::string mix_hex2 = s2.substr(st, en - st);
        pload2.mix = std::strtoull(mix_hex2.c_str() + 2, nullptr, 16);
        auto l2 = s2.find("\"last_obs\"");
        auto a2 = s2.find('[', l2);
        auto b2 = s2.find(']', a2);
        std::size_t wi = 0;
        auto pp = a2 + 1;
        while (pp < b2 && wi < pload2.last_obs.Words().size()) {
          auto q = s2.find('"', pp);
          if (q == std::string::npos || q >= b2) break;
          auto qn = s2.find('"', q + 1);
          if (qn == std::string::npos || qn > b2) break;
    ...
    const std::size_t first_chk_index =
        static_cast<std::size_t>((N / K));  // 1-based multiples; index is 0-based
    ASSERT_EQ(resumed_chk.size(), total_checkpoints - first_chk_index);
    for (std::size_t i = 0; i < resumed_chk.size(); ++i) {
      EXPECT_EQ(Hex64(resumed_chk[i]), exp_chk_hex[first_chk_index + i])
          << "N=" << N << ", chk i=" << i;
    }
